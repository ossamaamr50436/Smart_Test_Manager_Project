...
# الجزء الخامس — تخفيف شروط كلمة المرور

> تاريخ التنفيذ: السبت 2026-09-12 — المطلب الوحيد: **5 مدخلات على الأقل** — أي مزيج من
> أي 5 رموز (لا حرف كبير/صغير/رقم/رمز مطلوب).

## 30) التغييرات على المصادقة والتحقق

| الملف | الإجراء |
| :--- | :--- |
| `lib/validations/user.ts` | `passwordSchema` أصبح `min(5, "كلمة المرور يجب أن تكون 5 أحرف/رموز على الأقل")` + `max(128)` فقط — **حُذفت كل `.regex(...)`** (A-Z / a-z / 0-9). |
| `lib/actions/admin-panel-actions.ts` | `createAdminUser`: تحقق عبر `createUserSchema` الجديد تلقائياً. `resetAdminUserPassword`: حُذف فحص `/A-Z/`,`/a-z/`,`/0-9/` — اكتفاء بـ `newPassword.length >= 5` مع رسالة "5 أحرف/رموز على الأقل". |
| `lib/actions/examiner-actions.ts` | `createExaminer` و `resetExaminerPassword` — يستخدمان `passwordSchema` (التبسيط يصل إليهما تلقائياً). |
| `lib/actions/entity-actions.ts` | `createInstitutionBySpecialist` — كلمة المرور = رقم التصريح بفحص طول فقط (3–50) — لا regex صارم أصلاً. |
| `lib/actions/change-password-actions.ts` | `changeMyPassword` — يمر عبر `passwordSchema` الجديد؛ أُبقيَت التحققات المنطقية فقط: الحالية مطلوبة، الجديدة ≠ الحالية، تطابق التأكيد. |
| `auth.ts` | `credentialsSchema.password` = `z.string().min(1, "كلمة المرور مطلوبة")` — لم يتغير (لا تقييد طول على الدخول أصلاً). |

## 31) رسائل الواجهة (UI helpers)

استُبدل أي نص يذكر "حرف كبير / حرف صغير / رقم" بجانب "كلمة المرور" بالنص الموحّد:

> **"كلمة المرور: 5 أحرف/رموز على الأقل (لا شروط أخرى)."**

| الملف | الموضع |
| :--- | :--- |
| `components/admin/admin-users-manager.tsx` | تلميح نموذج الإنشاء + تلميح إعادة التعيين (موضعان) |
| `components/auth/change-password-form.tsx` | تلميح حقل كلمة المرور الجديدة |
| `components/specialist/teachers-manager.tsx` | تلميح كلمة المرور المؤقتة + تلميح إعادة التعيين (موضعان) |

## 32) التوثيق المُحدَّث

- `ACCOUNTS_REPORT.md` — قسم "قواعد كلمة المرور المطبّقة" أصبح: 5 أحرف/رموز كحد أدنى،
  128 حد أقصى، **لا شروط أخرى**.
- `HANDOVER_REPORT.md` — المرحلة 3: سياسة كلمة المرور (5 أحرف/رموز بلا شروط أخرى).
- `docs/` و `README.md` — لا نصوص تصف الشروط القديمة (تم التحقق بالبحث الشامل).

## 33) التحقق النهائي والامتثال

```powershell
pnpm typecheck   # OK — بدون أخطاء (tsc --noEmit)
pnpm build       # OK — 45/45 صفحة بلا أخطاء
```

حالات النجاح المطلوبة:
- إنشاء مستخدم بكلمة `abcde` → نجاح ✅ (تجاوز `min(5)`)
- إنشاء مستخدم بكلمة `12345` → نجاح ✅ (لا شرط رقم/حرف)
- إنشاء مستخدم بكلمة `!@#$%` → نجاح ✅ (لا شرط رمز/حرف)
- إنشاء مستخدم بكلمة `abc` (3 أحرف) → رفض برسالة عربية "كلمة المرور يجب أن تكون 5 أحرف/رموز على الأقل" ✅
- تغيير كلمة المرور بنفس الشروط ✅

الالتزام بالقيود: لا push إلى GitHub، لا حذف ملفات، لا `any`/`as unknown as`.