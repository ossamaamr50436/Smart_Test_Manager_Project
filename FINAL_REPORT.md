...
## 16) إصلاحات ما بعد النشر — الجولة السابعة (المهام A–F + 1–6 + 10)

مهام تشغيلية كاملة (مهام B..10) نُفّذت بعملية تحقق صارمة لكل مهمة:
`pnpm typecheck` + `pnpm lint` لكل commit، وتحقق برمجي إضافي للبيانات حيث لزم.

| المهمة | المحتوى | Commit |
| --- | --- | --- |
| **A** | إصلاح ظهور المعلمين في نموذج إنشاء اللجنة (قائمة فارغة) | `3a288ae` |
| **B** | التحقق من مسارات إنشاء المؤسسة (Verify-only — بلا Commit) | — |
| **C** | `PhoneInput` بمعرف الدولة + العلم (الافتراضي SA 966) عبر المنصة | `1bf198e` |
| **D** | حقل ترشيح التلميذ في شهادات الأخصائي + اختيار المؤسسة + تغيير مخطط `Student.certificateSourceId` | `2fac201` |
| **E** | صفحة تفاصيل الطالب `institution/students/[id]` بخط زمني كامل (تسلسل + مؤشرات) + حقول توقيت المخطط | `5d7ca26` |
| **F** | صفحة شهادات المؤسسة `institution/certificates` (فلاتر + إحصائيات) | `ed62f2d` |
| **1** | إخفاء إعدادات المنصة العامة (الاسم/الشعار/الدعم) عن مشرف المؤسسة | `86c4c53` |
| **2** | حذف المواسم `admin/seasons` مع معالجة العلاقات | `d2a656e` |
| **3** | بنك الأسئلة الدائم للأخصائي والمشرف (سؤال + شاشة كاملة) | `beff47b` |
| **4** | تنبيه المنصة أعلى الشاشة لجميع المستخدمين (دور إعادة الظهور) | `ab0f219` |
| **5** | إخفاء الإشعار فور قراءته (Pusher + تفاؤلي) | `0aa499e` |
| **6** | عزل تنبيهات الأمان لـ SUPER_ADMIN فقط + `guardSuperAdminPage` | `0eb9464` |
| **10** | تعميم العلامة التجارية (إزالة إشارات جمعية/فرع المدينة) | `11f84d7` |

### تفاصيل تلخيصية لكل مهمة

1. **تنبيه المنصة (4/5):** `Notification.senderId` + علاقة `NotificationSender`
   (migration `20260914150000_add_notification_sender`)؛ `notifyTenantAdmins`/
   `notifyAllTenantUsers`/`broadcastToAllUsers` تسجل `senderId: user.id`؛
   `getPlatformAlert()/dismissPlatformAlert()`؛ شريط `platform-alert-banner`
   لاصق (sticky، amber، ×) فوق لوحة التحكم. إخفاء تفاؤلي عند القراءة عبر قناة
   Pusher `notification:read` على `private-user-{id}` وتحديث الشارة/القائمة
   فوراً (لا يتطلب إعادة تحميل).

2. **عزل التنبيهات الأمنية (6):** `raiseSecurityAlert` تطبع التفاصيل موحّدة
   (`alertType/message/ip/userId/tenantId/operation/timestamp`)؛
   `guardSuperAdminPage` يرفع `suspicious_access` ويعيد توجيه غير
   SUPER_ADMIN من كل صفحات `/super-admin`؛ سكربت تحقق (`security-isolation-test.ts`)
   أنشأ مستأجرين A/B و9/9 عبارات ناجحة (الحجب، عدم التسريب، سجل التدقيق)
   ثم حُذف السكربت ونُظفت بيانات التجربة.

3. **تعميم العلامة التجارية (10):** إزالة كل نص/شعار يفيد الانتماء لجمعية/فرع
   معينة مع الإبقاء على المنطق الوظيفي: حماية `madina-quran` وبياناتها و100
   نموذجها سليمة. الشهادات تستخدم `organizationName` من مؤسسة الطالب مع افتراضي
   عام «منصة إدارة الاختبارات الذكية»؛ ترويسات/JSON-LD في `app/layout.tsx`
   معمَّمة؛ إعادة صياغة صفحة التعليم والدور؛ شعار عام جديد (قبعة تخرج تركوازية)
   بعيداً عن أي شعار جمعية؛ `manifest.json` معمَّم.

### التحقق النهائي الشامل (Task 7)

| البند | النتيجة |
| --- | --- |
| `pnpm typecheck` | ✅ 0 أخطاء |
| `pnpm lint` | ✅ نظيف |
| `pnpm build` | ✅ نجاح — البناء الكامل (22 صفحة static + كل المسارات الديناميكية) |
| `npx prisma validate` | ✅ schema صالح |
| `npx prisma migrate status` | ✅ migration الصفري `enable_row_level_security` ما زال غير مطبَّق |
| `madina-quran` / 100 نموذج | ✅ سليمة |
| عزل المستأجرين (App-level) | ✅ قائم بالكامل (فحوصات سابقة 18/18 + فحص المهام 9/9) |
| فحص العلامة التجارية | ✅ لا إشارات لـ«جمعية تعليم القرآن» أو «فرع المدينة» في `app/` و`components/` و`lib/` |

> Commits (لم تُدفع إلى GitHub — رفع يدوي من قِبل المالك):
> - `feat/security/fix/refactor` لسلسلة `a-b-c-d-e-f-1-2-3-4-5-6-10` كما في الجدول أعلاه
> - `docs: add round 7 report` (هذا التقرير)

---

*نهاية التقرير — المستند سيجري مراجعته ورَّفعه يدوياً، لا يُدفع تلقائياً إلى GitHub.*