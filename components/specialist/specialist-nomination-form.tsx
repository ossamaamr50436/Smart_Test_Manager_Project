...
import { createStudentApplicationBySpecialist } from "@/lib/actions/student-actions";
import {
  BRANCHES,
  NATIONALITIES,
...
export function SpecialistNominationForm({
  institutions,
}: {
  ...
  async function onSubmit(data: StudentApplicationInput) {
    ...
    setLoading(true);
    try {
      const result = await createStudentApplicationBySpecialist(
        { ...data, institutionId },
        applicationFile
          ? {
      ...
      if (result.success) {
        router.push("/test-specialist/requests");
        router.refresh();
      } else {
  ...
  return (
    <Card className="w-full max-w-2xl">
      <CardHeader>
        <CardTitle>ترشيح طالب من الأخصائي</CardTitle>
        <CardDescription>
          اختر الجهة التعليمية وأدخل بيانات الطالب — الحالة تبدأ مقبولة مباشرة
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={institutionId} onValueChange={setInstitutionId}>
              <SelectTrigger>
                <SelectValue placeholder="اختر الجهة التعليمية" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="age" type="number" min={4} max={18} placeholder="مثال: 12" {...register("age")} />
              {errors.age && (
          ...
          <div className="space-y-2 rounded-lg border p-4">
            <Label htmlFor="applicationFile">
              نموذج اختبار الطالب (PDF) — اختياري
            </Label>
            <Input
              id="applicationFile"
              type="file"
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? "جارٍ الإرسال..." : "ترشيح الطالب"}
            </Button>
          </div>
        </form>
      </CardContent>
