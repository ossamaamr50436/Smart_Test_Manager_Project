...
| `APPROVED` | اعتماد المختبر لتقييمه (حالة نهائية لتقييم المختبر) |
| `ACCEPTED` | اعتماد الأخصائي إدارياً/نهائياً |

## سير الاعتماد المستقل — المنطق في `approveAssessment`

1. `approveAssessment` يعتمد تقييم المختبر الحالي فقط ويعرّف الطالب بـ `COMPLETED` (جاهز لمراجعة الأخصائي).
2. `getAssessmentState` يعيد تقييمات جميع مختبرَي اللجنة **منفصلة** ليعرضها الأخصائي.
3. لا يوجد ترتيب اعتماد حسب العمر — كل مختبر يعتمد مستقلاً.
