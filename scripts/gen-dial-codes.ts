...
// خريطة رموز الاتصال حسب ISO (ITU E.164) — رموز بادئة متعددة البلدان مكررة
...
const rows = flags
  .map((f) => ({ code: f.code, dial: DIAL[f.code] ?? null }))
  .filter((r) => r.dial);

const missing = flags.filter((f) => !DIAL[f.code]).map((f) => f.code);
console.log("missing codes:", missing.join(","));

const byCode = Object.fromEntries(rows.map((r) => [r.code, r.dial]));
console.log(`generated ${Object.keys(byCode).length} entries`);
require("fs").writeFileSync(
  "scripts/data/dial-codes.json",
  JSON.stringify(byCode),
  "utf8"
);
console.log("written scripts/data/dial-codes.json");